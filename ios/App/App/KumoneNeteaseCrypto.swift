...
/**
 * Adapted from Kumone's NeteaseCrypto.swift.
 *
 * Kumone is LGPL-3.0-only; see third_party/kumone/LICENSE and
 * THIRD_PARTY_NOTICES.md. Keeping this small crypto layer native avoids
 * shipping the NetEase private request keys through the browser bundle.
 */
enum KumoneNeteaseCrypto {
    ...
    private static func aes128(_ data: Data, key: String, cbcIV: String?) -> Data {
        ...
        precondition(status == kCCSuccess, "Kumone AES encryption failed: \(status)")
